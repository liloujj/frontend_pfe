...
export default function ViewScanPage() {
  ...
  useEffect(() => {
    // Dans une vraie application, vous feriez un appel API pour récupérer les données du scan
    // Ici, nous simulons la récupération des données
    if (id) {
      const scanData = medicalDocuments.find((doc) => doc.id.toString() === id)
      if (scanData) {
        setScan(scanData)
      } else {
        // Si le scan n'est pas trouvé, utilisez le premier scan comme exemple
        setScan(medicalDocuments[0])
      }
    } else {
      // Si aucun ID n'est fourni, utilisez le premier scan comme exemple
      setScan(medicalDocuments[0])
    }
  }, [id])
  ...
  return (
    <div className="container mx-auto py-8 px-4">
      <div className="flex items-center mb-6">
        ...
        <h1 className="text-3xl font-bold text-white">{scan.name}</h1>
        <Badge className="ml-4">{scan.date}</Badge>
            ...
            <CardContent>
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-white">{scan.type || "X-Ray"}</p>
                </div>
                <div>
